...
#include "vilo/vikit/patch_score.h"
...
void getWarpMatrixAffine(const vilo::AbstractCamera& cam_ref,
  ...
  for (int y=0; y<patch_size; ++y)
    for (int x=0; x<patch_size; ++x, ++patch_ptr)
    {
      ...
      if (px[0]<0 || px[1]<0 || px[0]>=img_ref.cols-1 || px[1]>=img_ref.rows-1)
        ...
        *patch_ptr = (uint8_t) vilo::interpolateMat_8u(img_ref, px[0], px[1]);
    }
}
    ...
    for (int y=0; y<patch_size; ++y)
        ...
        {   
            ...
            if (px[0]<0 || px[1]<0 || px[0]>=img_ref.cols-1 || px[1]>=img_ref.rows-1)
                ...
                *patch_ptr = vilo::interpolateMat_8u(img_ref, px[0], px[1]);
        }
...
bool Matcher::findMatchDirect(Point& pt, Frame& cur_frame, Vector2d& px_cur)
{
    ...
    Sophus::SE3 Tfw_cur = cur_frame.getFramePose();
    Sophus::SE3 Tfw_ref = ref_ftr_->frame->getFramePose();
    Sophus::SE3 T_c_r = Tfw_cur * Tfw_ref.inverse();

    if(ref_ftr_->frame->id_ == pt.hostFeature_->frame->id_)
    {
        double idist = pt.getPointIdist();
    ...
    {
        Eigen::Vector3d p_w = pt.getPointPose();
        warp::getWarpMatrixAffine(
            *ref_ftr_->frame->cam_, *cur_frame.cam_, ref_ftr_->px, ref_ftr_->f,
            (ref_ftr_->frame->pos() - p_w).norm(), T_c_r, ref_ftr_->level, A_cur_ref_);
    }
...
{
    Sophus::SE3 Twf_seed = seed.ftr->frame->getFramePose().inverse();
    Vector3d seed_pos(Twf_seed * (1.0/seed.mu * seed.ftr->f) );
    ...
    Sophus::SE3 Tfw_cur = cur_frame.getFramePose();
    Sophus::SE3 T_c_r = Tfw_cur * Twf_seed;
...
{
    Sophus::SE3 Tfw_cur = cur_frame.getFramePose();
    Sophus::SE3 Tfw_ref = ref_frame.getFramePose();
    Sophus::SE3 T_cur_ref = Tfw_cur * Tfw_ref.inverse();

    int zmssd_best = PatchScore::threshold();
    Vector2d uv_best;

    Vector2d A = vilo::project2d(T_cur_ref * (ref_ftr.f*d_min));
    Vector2d B = vilo::project2d(T_cur_ref * (ref_ftr.f*d_max));
    epi_dir_ = A - B;
  ...
  if(zmssd_best < PatchScore::threshold())
  {
    ...
    px_cur_ = cur_frame.cam_->world2cam(uv_best);
    if(depthFromTriangulation(T_cur_ref, ref_ftr.f, vilo::unproject2d(uv_best).normalized(), depth))
      return true;
  }
  return false;
}
    ...
    double& result_depth, Vector2i& EPL_start, Vector2i& EPL_end)
{
    ...
    if(std::isinf(eplLength) || incx==0 || incy==0)
    {
        return -1;
    }
    ...
    vilo::patch_score::ZMNCC_F<halfpatch_size_> patchScore(patch_f_);
    ...
    const double d_min, const double d_max, double& depth)
{
    ...
    if(abs(bestCounter - secondCounter) > 1.0f && MIN_DISTANCE_ERROR_STEREO * zmncc_second > zmncc_best)
        return false;
