    ...
    size_t& num_obs)
{
    ...
    vilo::robust_cost::HuberWeightFunction weight_function;
    ...
    for(Features::iterator it=frame->fts_.begin(); it!=frame->fts_.end(); ++it)
    {
        ...
        Vector2d e = vilo::project2d((*it)->f) - vilo::project2d(pTarget);
        e *= 1.0 / (1<<(*it)->level);

        int pt_type = (*it)->point->getPointState();
        if((*it)->type == Feature::EDGELET)
        {
            ...
            if(pt_type == Point::TYPE_TEMPORARY) 
                weight *= 0.5;
    ...
    for(size_t iter=0; iter<n_iter; iter++)
    {
        ...
        do
        {
            ...
            idx_host = 0;
            Sophus::SE3 Tfw_t_tmp = frame->getFramePose();
            for(Features::iterator it=frame->fts_.begin(); it!=frame->fts_.end(); ++it)
            {
                if((*it)->point == NULL) continue;
                ...
                Vector3d pHost = v_host[idx_host];
                Sophus::SE3 Tfw_h = host->getFramePose();
                Sophus::SE3 Tth = Tfw_t_tmp * Tfw_h.inverse();
            ...
            if(!(bool) std::isnan((double)dT[0]))
            {
                ...
                for(Features::iterator it=frame->fts_.begin(); it!=frame->fts_.end(); ++it)
                {
                    ...
                    Vector3d pHost = v_host[idx_host];
                    Sophus::SE3 Tfw_h = host->getFramePose();
                    Sophus::SE3 Tth = T_new * Tfw_h.inverse();
                    Vector3d pTarget = Tth * pHost;
            ...
            if(rho>0)
            {
                frame->setFramePose(T_new);
                chi2 = new_chi2;
                stop = vilo::norm_max(dT) <= EPS;
    ...
    if(!chi2_vec_init.empty())
        error_init = sqrt(vilo::getMedian(chi2_vec_init))*frame->cam_->errorMultiplier2();
    if(!chi2_vec_final.empty())
        error_final = sqrt(vilo::getMedian(chi2_vec_final))*frame->cam_->errorMultiplier2();
